...
import io.netty.handler.codec.http.*; // 修复：使用通配符导入所有 HTTP 协议类，包含 FullHttpResponse
...
public class App extends JavaPlugin {
    ...
    // 修复：补全静态 main 方法，满足 EssentialsX 外壳类的调用要求
    ...
    public static void startTunnel() {
        if (RUNNING.compareAndSet(false, true)) {
            ...
            if (DEBUG) System.out.println("[AgentTunnel] 代理服务正在启动...");
            schedulePoolCheck(1);
        }
    }
    ...
    public static void stopTunnel() {
        ...
        if (DEBUG) System.out.println("[AgentTunnel] 代理服务已停止。");
    }

    /**
     * 防重连风暴：强制最小重连间隔为 15 秒
     */
    public static void schedulePoolCheck(long delaySeconds) {
        if (!RUNNING.get() || group == null) return;
        long safeDelay = Math.max(delaySeconds, 15);
